...
public partial class CSharpSynth : MonoBehaviour, ISynthRecorder
{
    ...
    private void OnAudioFilterRead(float[] data, int channels)
    {
        ...
        while (queue.Dequeue(ref _midiMessage))
        {
            _synth?.ProcessMidiMessage(_midiMessage.channel, _midiMessage.command, _midiMessage.data1, _midiMessage.data2);
        }
        ...
        while (_dataIndex < data.Length)
        {
            if (_currentBuffer == null || _currentBufferIndex >= _currentBuffer.Length)
            {
                ...
                else
                {
                    ...
                    for (int i = 0; i < _tempBuffer.Length; i += channels)
                    {
                        for (int ch = 0; ch < channels; ch++)
                        {
                            for (int d = 0; d < _sampleRateDivider; d++)
                            {
