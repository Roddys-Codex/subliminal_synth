...
namespace jp.kshoji.unity.midi.sample
{
    public class MidiSampleScene : MonoBehaviour, IMidiAllEventsHandler, IMidiDeviceEventHandler
    {
        ...
        private void Awake()
        {
            ...
            {
                //ipAddressText.text = ipEntry.AddressList.GetValue(0).ToString();
            }

            //Button serverBtn = serverStartBtn.GetComponent<Button>();
            //serverImage = serverStartBtn.GetComponent<Image>();
            //serverBtn.onClick.AddListener(initiateRtpServer);

            //Button stopBtn = stopRTPmidiBtn.GetComponent<Button>();
            //stopBtn.onClick.AddListener(stopRtpServer);

            //Button connectBtn = connectToHostBtn.GetComponent<Button>();
            //connectBtn.onClick.AddListener(connectToRtpServer);

            //Button sendMidiOnbutton = sendOnMidiBtn.GetComponent<Button>();
            //sendMidiOnbutton.onClick.AddListener(sendMidiOnEvent);

            //Button sendMidiOffbutton = sendOffMidiBtn.GetComponent<Button>();
            //sendMidiOffbutton.onClick.AddListener(sendMidiOffEvent);

            //guiScale = (Screen.width > Screen.height) ? Screen.width / 1024f : Screen.height / 1024f;
            Debug.Log("RegisteringEventHandleObject");
            MidiManager.Instance.RegisterEventHandleObject(gameObject);

            Debug.Log("Initializing Midi");
        ...
        public void initiateRtpServer()
        {
            //serverImage.color = new Color32(((byte)random.Next(1, 255)), ((byte)random.Next(1, 255)), ((byte)random.Next(1, 255)), ((byte)random.Next(1, 255)));;
            MidiManager.Instance.StartRtpMidiServer("MIDINET", 5004);
            Debug.Log("SERVER BUTTON PRESSED");
        }

        private void stopRtpServer()
        {
            MidiManager.Instance.StopRtpMidi(5004);
            Debug.Log("SERVER STOPPED");
        }

        public void connectToRtpServer()
        {
            Debug.Log("CONNECTING TO EXTERNAL MIDI NETWORK");
            MidiManager.Instance.ConnectToRtpMidiServer("MidiNetwork", 5004, new IPEndPoint(IPAddress.Parse("192.168.0.17"), int.Parse("5004")));
...
//        }
        ...
        {
            receivedMidiMessages.Add($"MIDI Input device detached. deviceId: {deviceId}, name: {MidiManager.Instance.GetDeviceName(deviceId)}");
        }

        public void OnMidiOutputDeviceDetached(string deviceId)
        {
            receivedMidiMessages.Add($"MIDI Output device detached. deviceId: {deviceId}, name: {MidiManager.Instance.GetDeviceName(deviceId)}");
        }
