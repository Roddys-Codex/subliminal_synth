    ...
    private static extern void sendMidiReset(string deviceId);
        ...
        public static MidiManager Instance
        {
            get
            {
                ...
                lock (LockObject)
                {
                    instance = new GameObject("MidiManager").AddComponent<MidiManager>();
#if (!UNITY_IOS && !UNITY_WEBGL) || UNITY_EDITOR
...
#if ENABLE_IL2CPP
                        Instance.bleMidiPlugin.Call("initialize", context, new BleMidiDeviceConnectionListener(), new BleMidiInputEventListener());
#else
                        Instance.bleMidiPlugin.Call("initialize", context);
#endif
                        if (Thread.CurrentThread != mainThread)
                        {
                            AndroidJNI.DetachCurrentThread();
        ...
        public void StartRtpMidiServer(string sessionName, int listenPort)
        {
            ...
            lock (rtpMidiServers)
            {
                ...
                if (rtpMidiServers.ContainsKey(listenPort))
                {
        ...
        public bool IsRtpMidiRunning(int listenPort)
        {
            ...
            lock (rtpMidiServers)
            {
                ...
                var rtpMidiServer = rtpMidiServers[listenPort];
...
#elif UNITY_IOS
            throw new NotImplementedException("iOS platform isn't available");
#endif
        }
        ...
        {
            if (!midiDeviceEventHandlers.Contains(eventHandler))
            {
        ...
        public string GetDeviceName(string deviceId)
        {
            ...
            {
                return deviceNameCache[deviceId];
            }
        ...
        public void SendMidiNoteOn(string deviceId, int group, int channel, int note, int velocity)
        {
            ...
            lock (rtpMidiServers)
            {
                ...
                if (rtpMidiServers.ContainsKey(port))
                {
                    var rtpMidiServer = rtpMidiServers[port];
        ...
        public void SendMidiProgramChange(string deviceId, int group, int channel, int program)
        {
            ...
            lock (rtpMidiServers)
            {
                ...
                {
                    var rtpMidiServer = rtpMidiServers[port];
                    rtpMidiServer.SendMidiProgramChange(deviceId, channel, program);
                }
            }
#endif
        }
        ...
        public void SendMidiSystemExclusive(string deviceId, int group, byte[] sysEx)
        {
            ...
            Instance.usbMidiPlugin.Call("sendMidiSystemExclusive", deviceId, group, sysEx);
            Instance.bleMidiPlugin.Call("sendMidiSystemExclusive", deviceId, sysEx);
            if (Thread.CurrentThread != mainThread)
            {
                AndroidJNI.DetachCurrentThread();
            }
            ...
            foreach (var midiDeviceEventHandler in midiDeviceEventHandlers)
            {
                ...
                ExecuteEvents.Execute<IMidiPitchWheelEventHandler>(midiDeviceEventHandler,
                    eventData, (eventHandler, baseEventData) =>
                    {
                        ...
                        {
                            var parsed = midiEventData.Message;
                            eventHandler.OnMidiPitchWheel(parsed.DeviceId, (int)parsed.Group,
                                (int)parsed.Messages[0], (int)parsed.Messages[1]);
                        }
                    });
            }
